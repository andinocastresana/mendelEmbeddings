...
| 26 | **Track 2b — Comparador con árbol genealógico (pedigree formal)**. Página nueva (no reemplaza al comparador 3-slot del Track 2a). Cada persona admite máx. 1 padre + 1 madre (pedigree formal). Fotos ilimitadas por persona pendiente decidir (MVP 1 foto/persona). Persistencia IndexedDB local por default + export/import (JSON con metadata del árbol + imágenes empaquetadas, ej. base64 o ZIP). Selección interactiva: el usuario clickea 2 personas del árbol y se computa el cosine on-demand. Las imágenes nunca salen del browser por default (export es opt-in). **Decisiones cerradas 2026-05-22**: (a) layout = pedigree clásico SVG (generaciones top-down, click P1 → click P2 → comparación); (b) 1 foto por persona en MVP, embedding cacheado por SHA-256; (c) persistencia = IndexedDB + export/import JSON+base64 (sin deps nuevas). **Decisiones cerradas 2026-05-23 al arrancar paso 5**: (d) trigger = toggle global "Modo comparación" (no implícito al click ni botón en panel detalle) — **reemplazado en iter B por Ctrl/Cmd+click**; (e) UI del resultado = panel "Comparación" lateral separado del panel detalle (flex row) — **reemplazado en iter B por línea naranja con cosine flotante sobre el SVG**; (f) comparaciones persistidas en IDB con schema propio (no efímero), historial filtrado por treeId con marker `⚠ stale` si la foto asociada cambió tras el cómputo (data persiste, UI de historial pendiente tras refactor iter B+C). **Decisiones cerradas 2026-05-23 en iter B+C**: (g) selección múltiple de N nodos (no par fijo) → `N*(N-1)/2` cosines + badges numerados; (h) click sobre label de cosine abre modal de tripleta con inferencia automática de roles desde el árbol; (i) handoff cross-tab al Comparador MVP vía localStorage one-shot (datos) + CustomEvent (señal de vista). **Plan 6 pasos**: (1) modelo `lib/genealogy.ts` + store `lib/treeStore.ts` con dedup por SHA-256 ✅ 2026-05-22; (2) UI lista (sin SVG) para validar persistencia e2e ✅ 2026-05-22 (smoke OK); (3) layout pedigree puro `lib/treeLayout.ts` ✅ 2026-05-22; (4) render SVG + drag-and-drop foto sobre nodo ✅ 2026-05-22; (5) selección de nodos + comparación on-demand reusando `lib/pipeline.ts` ✅ 2026-05-23 en 3 sub-iteraciones — base `bac4fac` (par P1/P2, panel lateral, historial IDB) + iter B `ec1d88f` (pedigree canónico con sib-bus, Ctrl+click, cosine sobre línea SVG) + iter C `ec1d88f` (multi-selección N nodos, modal de tripleta con roles inferidos, handoff cross-tab al Comparador MVP); smoke OK (cosine Bruno↔Mateo = 0.2302 determinístico); (6) export/import JSON+base64 + tab nuevo en `App.tsx` — **pendiente**. | Web | en progreso | 2026-05-21 | 2026-05-23 |
