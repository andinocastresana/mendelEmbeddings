...
| 25 | **Track 2a — MVP comparador anónimo browser**. Subtareas: (a) spike #003 alineación canónica JS ✅ commit `7af68d6`. (b) spike #004 detección JS + pipeline e2e ✅ commit `b3129af` (GLOBAL PASS sobre 4 imágenes diversas, mean cosine 0.98). (c) UI MVP del comparador — en progreso: ✅ refactor pipeline e2e extraído a `client/src/lib/pipeline.ts` (exports `computeEmbedding`, `initFaceLandmarker`, `initOnnxSession`, `loadImage`, `cosineSimilarity`, `imageDataToTensorRGB`, `MESH_INDICES_INSIGHTFACE_ORDER`); ✅ `SpikeDetection.tsx` refactorizado (v2.0→v2.1, 618→506 líneas) y revalidado con GLOBAL PASS post-refactor. **PRÓXIMO**: crear `client/src/Comparator.tsx` (2 inputs file + botón comparar → `computeEmbedding` ×2 → cosine + preview alineadas) y agregar tab en `App.tsx`. Imágenes nunca salen del browser. | Web | en progreso | 2026-05-21 | 2026-05-21 |
