...
| 27 | **Cleanup de recursos GPU/WASM en `Comparator.tsx`**. Bug latente detectado 2026-05-22 durante smoke del Track 2b: `Comparator.tsx` inicializa `FaceLandmarker` (MediaPipe GPU) y `ort.InferenceSession` (ONNX WebGPU/WASM) en `ensureInit` pero no los libera al desmontar el componente. Los contextos GPU/WASM quedan vivos hasta que muere el proceso del browser, contribuyendo al calentamiento sostenido. Fix: agregar `useEffect` con cleanup que llame `landmarkerRef.current?.close()` y, si la API lo soporta, `sessionRef.current?.release()`. Validar que `tab === 'comparator'` toggle libera la GPU (chequeable con Chrome Task Manager). Aplicable también a los Spikes que inicializan los mismos motores. | Web | pendiente | 2026-05-22 | 2026-05-22 |
