...
| 26 | **Track 2b — Comparador con árbol genealógico (pedigree formal)**. Página nueva (no reemplaza al comparador 3-slot del Track 2a). Cada persona admite máx. 1 padre + 1 madre (pedigree formal). Fotos ilimitadas por persona pendiente decidir (MVP 1 foto/persona). Persistencia IndexedDB local por default + export/import (JSON con metadata del árbol + imágenes empaquetadas, ej. base64 o ZIP). Selección interactiva: el usuario clickea 2 personas del árbol y se computa el cosine on-demand. Las imágenes nunca salen del browser por default (export es opt-in). **Decisiones cerradas 2026-05-22**: (a) layout = pedigree clásico SVG (generaciones top-down, click P1 → click P2 → comparación); (b) 1 foto por persona en MVP, embedding cacheado por SHA-256; (c) persistencia = IndexedDB + export/import JSON+base64 (sin deps nuevas). **Decisiones cerradas 2026-05-23 al arrancar paso 5**: (d) trigger = toggle global "Modo comparación" (no implícito al click ni botón en panel detalle); (e) UI del resultado = panel "Comparación" lateral separado del panel detalle (flex row); (f) comparaciones persistidas en IDB con schema propio (no efímero), historial filtrado por treeId con marker `⚠ stale` si la foto asociada cambió tras el cómputo. **Plan 6 pasos**: (1) modelo `lib/genealogy.ts` + store `lib/treeStore.ts` con dedup por SHA-256 ✅ 2026-05-22; (2) UI lista (sin SVG) para validar persistencia e2e ✅ 2026-05-22 (smoke OK); (3) layout pedigree puro `lib/treeLayout.ts` ✅ 2026-05-22; (4) render SVG + drag-and-drop foto sobre nodo ✅ 2026-05-22; (5) selección 2 nodos + comparación on-demand reusando `lib/pipeline.ts` ✅ 2026-05-23 (smoke OK, cosine Bruno↔Mateo = 0.2302 determinístico, historial persistido); (6) export/import JSON+base64 + tab nuevo en `App.tsx`. | Web | en progreso | 2026-05-21 | 2026-05-23 |
